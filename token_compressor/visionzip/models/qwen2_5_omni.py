...
from token_compressor.visionzip import compress_video_features
...
def _maybe_init_stats(self) -> None:
    if hasattr(self, "_omnicom2_stats"):
        return
    self._omnicom2_stats = {
...
def Qwen2_5_OmniThinker_forward(
    ...
    **kwargs,
    ...
    """Patched forward that enables OmniCom2 token compression for Qwen2.5-Omni."""
    ...
    if input_features is not None:
        ...
        audio_features = _unpack_audio_outputs(audio_outputs)
        ...
        if os.getenv("BASELINE_USE_REAL_ATTENTION", "1") != "0":
            try:
                ...
                if os.getenv("BASELINE_ATTENTION_DEBUG", "0") == "1":
                    print(f"[visionzip] real attention extraction failed, falling back to norm: {exc}")
    ...
    if compression_on:
        stats_enabled = (
            os.getenv(_OMNICOM2_TOKEN_STATS_ENV, "0") == "1"
            or os.getenv("TOKEN_STATS", "0") == "1"
        )
        stats_case = (
            os.getenv(_OMNICOM2_TOKEN_STATS_CASE_ENV, "0") == "1"
        ...
        kept_audio_features = audio_features
        kept_audio_positions = None
        if audio_features is not None and audio_mask is not None:
            audio_token_positions = audio_mask[..., 0][0].nonzero(as_tuple=False).squeeze(-1)
            kept_audio_positions = audio_token_positions

        all_positions = torch.arange(inputs_embeds.shape[1], device=inputs_embeds.device)
        reserved_positions = torch.zeros_like(all_positions, dtype=torch.bool)
        reserved_positions[video_token_positions] = True
        if audio_mask is not None:
            audio_token_positions = audio_mask[..., 0][0].nonzero(as_tuple=False).squeeze(-1)
            reserved_positions[audio_token_positions] = True
        base_positions = all_positions[~reserved_positions]
        keep_parts = [base_positions, kept_video_positions]
        if kept_audio_positions is not None:
            keep_parts.append(kept_audio_positions)
        keep_token_indices = torch.cat(keep_parts).sort().values
        ...
        if audio_mask is not None and kept_audio_features is not None:
            inputs_embeds = inputs_embeds.masked_scatter(audio_mask, kept_audio_features.to(inputs_embeds.dtype))

        if stats_enabled or stats_case:
            post_stats = _count_tokens(input_ids, attention_mask, self.config)
            for key in self._omnicom2_stats["pre"]:
                self._omnicom2_stats["pre"][key] += int(pre_stats[key].sum().item())
                self._omnicom2_stats["post"][key] += int(post_stats[key].sum().item())
            self._omnicom2_stats["samples"] += int(pre_stats["total"].shape[0])
            if stats_case:
                self._omnicom2_last_stats = [
                    {
